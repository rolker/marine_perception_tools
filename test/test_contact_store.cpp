...
using marine_perception_tools::export_contacts_geojson;
...
TEST(ContactStore, GeoJsonExportWritesResolvedSkipsUnreferenced)
{
  ...
  const std::string path = std::string(testing::TempDir()) + "mpt_contacts_test.geojson";
...
TEST(ContactStore, GeoJsonExportEmptyOnNoGeo)
{
  ...
  const std::string path = std::string(testing::TempDir()) + "mpt_contacts_nogeo.geojson";
  const auto r = export_contacts_geojson(contacts, path);
