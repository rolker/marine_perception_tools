...
const char * offline_projection_caveat()
{
  return
    ...
    "term from the depth-budget term, caps what is left at that same "
    "horizontal term, and floors it at the cell size. The cap is a CEILING of "
    "at least ~5.15 m (4 m² is a floor on the horizontal variance, and the "
    "other horizontal terms add to it), and it is reached only at coarse "
    "cells: at fine cells — below ~0.75 m — each sounding's influence radius "
    "is one cell whatever the IHO budget; at a 1 m cell it is one cell for "
    "order 1a or tighter and ~1.5-2 m at order 2; the ceiling is not reached "
    "until roughly 2 m cells. So a coarse cell costs a few times the one-cell "
    "spread loop, not a hundred times; (2) lever "
    "arms are zero and "
