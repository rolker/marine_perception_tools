...
| *(same package)* | C++ (Qt5) | Also builds `survey_explorer` — the offline georeferenced sidescan/MBES viewer + target tracker AND tile-granular survey-data explorer in one window ([#7](https://github.com/rolker/marine_perception_tools/issues/7)/[#8](https://github.com/rolker/marine_perception_tools/issues/8)/[#24](https://github.com/rolker/marine_perception_tools/issues/24); `sidescan_target_viewer` remains as an exec shim, [#22](https://github.com/rolker/marine_perception_tools/issues/22)): geo map pane, sidescan waterfall, MBES backscatter, water-column echogram, 3D point cloud, contact marking. CLI: `survey_explorer [bag] [--start T --end T] [--index survey_index.db [--stores DIR]]` — the optional time window (UNIX ns or ISO-8601, UTC assumed) cues the scrub to that interval once indexing completes (jump-to-pass from `survey_index_query`, [#17](https://github.com/rolker/marine_perception_tools/issues/17)). With `--index` the map IS the survey index ([#19](https://github.com/rolker/marine_perception_tools/issues/19)/[#24](https://github.com/rolker/marine_perception_tools/issues/24)): store-tile basemap (GGGS GeoTIFFs via `marine_tiled_raster_store`; `--stores` defaults to `<index dir>/depths/processed`), per-bag nav track polylines (direction comes from the time-bar position arrow), one geographic **region** drawn by left-drag (#42; modifiers retired, middle-click recentres and seeks, middle-drag pans, right-click menu clears). The region auto-loads every `mbes-bathy` pass of the index tiles it covers into the 3D cloud ([#21](https://github.com/rolker/marine_perception_tools/issues/21) loader — windowed, topic-filtered bag reads; one golden-angle colour per pass + legend; cross-bag passes reprojected through the `earth` anchor into the first pass's map frame) and fills the **time bar** (ported from GeoZui4D's TimeControl; zoomable tick-ladder tape + full-extent scrollbar under the scrub controls; navigation-only). Committing a time on the bar cues the scrub — same-bag jumps directly, cross-bag re-opens through the selection; clicking a pass bar cues that pass. Bag opens go through the **bag-index cache** (`session_index_io`, `--cache-dir`, default `$XDG_CACHE_HOME/survey_explorer`; `--warm-cache` pre-builds the campaign): the SessionIndex is serialized keyed by bag size+mtime, so a cached open skips the whole-bag scan (90 s -> ~1 s on the June-15 bag). Bags are never written to. Map header: basemap layer picker + per-basemap colormap (percentile contrast via `basemap_contrast.hpp`) + track / index-grid / coastline / metric-grid declutter toggles; cloud legend rows are checkable (per-pass show/hide); 'clip to contact' + margin bounds a multi-pass load to the selected contact (per-bag geo->world clip in `mbes_pass_loader`); contacts delete via the list context menu; the map's static layers render through a per-view pixmap cache with sparse track/tile drawing, and GPU uploads are budgeted (4M-point cloud stride + 4000-row waterfall caps, both surfaced in the status line). An open bag places on the survey map through its probed earth anchor (`map_geo_anchor.hpp`); a bag without an earth reference hides its map layers rather than guessing. Note: a cache-miss bag open runs the full-bag *metadata* scan (TF + cumulative distance need the whole recording) once; only sample data is ever window-read. |
...
│   ├── sidescan_viewer_window.{hpp,cpp}  # survey_explorer's window: map + waterfalls + cloud, the four background workers and their cancel tokens
│   ├── worker_cancel.hpp       # supersede_token(): cancel a REPLACED background job (teardown is a separate token)
│   ├── world_layout.hpp        # ~/data/world layout: default index + stores dirs, preferred basemap layers
│   ├── basemap_lod.{hpp,cpp}   # store-tile basemap: level selection, residency, finer-over-coarser paint order (uma-ADR-0013 D5)
│   ├── coastline_data.hpp      # vendored Natural Earth 1:50m coastline + the scale fade that keeps it non-authoritative
│   ├── map_gesture.hpp         # pure gesture policy: which button does what (#42)
│   ├── view_animation.hpp      # easeInOutCubic recentre glide
│   ├── hover_geo_readout.hpp   # cursor lat/lon readout per pane
...
├── data/
│   └── coastline/              # vendored Natural Earth 1:50m polylines + regenerate.sh + provenance (sha256, counts)
├── test/
...
│   ├── test_mbes_projection.cpp  # GTest: the CUBE-lab projection helper — offline ProjectorParams (frames, minimum_range, library vessel/device), the vertical_error→vertical_variance rename, the per-ping/per-beam validity guard, and the caveat's wording
│   └── ... (one test_*.cpp per header above; see CMakeLists.txt for the full list)
